...
connect() {
    r=$(http --check-status PUT http://$HOST:$PORT/connect id=$ID psk=$PSK ip=$TRANSPORT)
    if [ $? -gt 0 ]; then
...
disconnect() {
    r=$(http --check-status DELETE http://$HOST:$PORT/connect id=$ID psk=$PSK)
    ret=0
    if [ $? -gt 0 ]; then
...
get_action() {
    r=$(http --check-status GET http://$HOST:$PORT/nextaction id=$ID)
    if [ $? -gt 0 ]; then
